...
{
    class Day08 : IAdventDayPuzzle
    {
        public string Description => @"As your submarine slowly makes its way through the cave system, you notice that the four-digit seven-segment displays in your submarine are malfunctioning; they must have been damaged during the escape. You'll be in a lot of trouble without them, so you'd better figure out what's wrong.";

        public DateTime Date => new DateTime(2021, 12, 8);
        ...
        {
            private readonly (string[] Patterns, string[] Output)[] _signalPatterns;

            public Solution1(string[] signalPatterns)
            {
                _signalPatterns = signalPatterns.Select(p =>
                {
                    var split = p.Trim().Split('|');
                    return (Patterns: split[0].Trim().Split(' '), Output: split[1].Trim().Split(' '));
                }).ToArray();
            }

            public string Description => @"In the output values, how many times do digits 1, 4, 7, or 8 appear?";
            public int Part => 1;
        ...
        internal class Solution2 : IAdventDaySolution
        {
            ...
            public Solution2(string[] signalPatterns)
            {
                _signalPatterns = signalPatterns.Select(p =>
                {
                    var split = p.Trim().Split('|');
                    return (
                        Patterns: split[0].Trim().Split(' ').Select(OrderCharactersInString).ToArray(),
                        Output: split[1].Trim().Split(' ').Select(OrderCharactersInString).ToArray()
                        );
                }).ToArray();
            }

            private static string OrderCharactersInString(string input)
            {
                return new string(input.OrderBy(c => c).ToArray());
            }

            public string Description => @"What do you get if you add up all of the output values?";
            public int Part => 1;
            public object Solve() => _signalPatterns.Sum(Solve);
            ...
            private int Solve((string[] Patterns, string[] Output) input)
            {
                ...
                while (patternsToDecrypt.TryDequeue(out var pattern) && !input.Output.All(o => mappings.ContainsKey(o)))
                {
                    var possibleSegments = SegmentToDigitMapping
                        ...
                        .Where(s => pattern
                                .All(p => s.Key
                                    .Select((c, i) => (IsOn: c == '1', Index: i))
                                    .Any(c => c.IsOn && segmentOptions[c.Index].Contains(p))))
                        .ToArray();

                    if (possibleSegments.Length == 1)
                    {
                        foreach (var c in possibleSegments[0].Key.Select((c, i) => (IsOn: c == '1', Index: i)))
                        {
                            segmentOptions[c.Index].RemoveAll(a => pattern.Contains(a) ^ c.IsOn);
                        }

                        foreach (var n in numberOptions.Select((n, i) => (Options: n, Number: i)))
                        {
                            n.Options.RemoveAll(a => a == pattern ^ n.Number == possibleSegments[0].Value);
                        }

                        mappings[pattern] = possibleSegments[0].Value;
                    }
                    else
                    {
                        foreach (var impossibleOption in possibleSegments
                            .Where(s => s.Key
                                .Select((c, i) => (IsOn: c == '1', Index: i))
                                .Where(c => c.IsOn)
                                .SelectMany(c => segmentOptions[c.Index])
                                .Where(c => !pattern.Contains(c))
                                .GroupBy(c => c)
                                .Any(c => c.Count() > 1)))
                        {
                            numberOptions[impossibleOption.Value].Remove(pattern);
                        }

                        patternsToDecrypt.Enqueue(pattern);
                    }
                }

                return input.Output.Reverse().Select((o, i) => mappings[o] * (int)Math.Pow(10, i)).Sum();
            }
        }

        internal static string[] Segments => new[]
        {
